import { thewordDAOABI, thewordDaoLogicV1Factory } from '@theword/sdk';
...
const abi = new utils.Interface(thewordDAOABI);
const thewordDaoContract = new thewordDaoLogicV1Factory().attach(config.addresses.thewordDAOProxy);
const proposalCreatedFilter = thewordDaoContract.filters?.ProposalCreated(
  null,
  null,
  null,
  null,
...
export const useHasVotedOnProposal = (proposalId: string | undefined): boolean => {
  ...
  const [receipt] =
    useContractCall<[any]>({
      abi,
      address: thewordDaoContract.address,
      method: 'getReceipt',
...
const useVotingDelay = (thewordDao: string): number | undefined => {
  const [blockDelay] =
    useContractCall<[EthersBN]>({
      abi,
      address: thewordDao,
...
export const useAllProposals = (): ProposalData => {
  ...
  const votingDelay = useVotingDelay(thewordDaoContract.address);
...
export const usePropose = () => {
  const { send: propose, state: proposeState } = useContractFunction(thewordDaoContract, 'propose');
