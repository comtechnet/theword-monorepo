...
import {
  ...
  thewordDaoLogicV1Harness,
  thewordDaoLogicV1Harness__factory as thewordDaoLogicV1HarnessFactory,
} from '../../../typechain';
...
async function deployGovernor(
  ...
  tokenAddress: string,
): Promise<thewordDaoLogicV1Harness> {
  const { address: govDelegateAddress } = await new thewordDaoLogicV1HarnessFactory(
    deployer,
  ).deploy();
  const params = [
    address(0),
  ...
  const { address: _govDelegatorAddress } = await (
    await ethers.getContractFactory('thewordDAOProxy', deployer)
  ).deploy(...params);

  return thewordDaoLogicV1HarnessFactory.connect(_govDelegatorAddress, deployer);
}
...
let gov: thewordDaoLogicV1Harness;
let targets: string[];
let values: string[];
...
async function reset() {
  ...
  token = await deploythewordToken(signers.deployer);

  await populateDescriptor(
    thewordDescriptorFactory.connect(await token.descriptor(), signers.deployer),
  );
...
describe('thewordDAO#castVote/2', () => {
  before(async () => {
  ...
  describe('We must revert if:', () => {
    ...
    it("There does not exist a proposal with matching proposal id where the current block number is between the proposal's start block (exclusive) and end block (inclusive)", async () => {
      await expect(gov.castVote(proposalId, 1)).revertedWith(
        'thewordDAO::castVoteInternal: voting is closed',
      );
    });
    ...
    it('Such proposal already has an entry in its voters set matching the sender', async () => {
      ...
      await expect(gov.connect(account0).castVote(proposalId, 1)).revertedWith(
        'thewordDAO::castVoteInternal: voter already voted',
      );
    });
  });
