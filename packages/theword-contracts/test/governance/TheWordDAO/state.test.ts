...
import {
  ...
  thewordDaoExecutorHarness,
  thewordDaoExecutorHarness__factory as thewordDaoExecutorHarnessFactory,
  thewordDaoImmutable,
  thewordDaoImmutable__factory as thewordDaoImmutableFactory,
} from '../../../typechain';
...
let gov: thewordDaoImmutable;
let timelock: thewordDaoExecutorHarness;
let delay: number;
...
async function makeProposal(
  ...
  timelock = await new thewordDaoExecutorHarnessFactory(deployer).deploy(deployer.address, delay);

  gov = await new thewordDaoImmutableFactory(deployer).deploy(
    timelock.address,
    token.address,
    address(0),
    deployer.address,
...
describe('thewordDAO#state/1', () => {
  ...
  it('Invalid for proposal not found', async () => {
    await makeProposal();
    await expect(gov.state(5)).revertedWith('thewordDAO::state: invalid proposal id');
  });
