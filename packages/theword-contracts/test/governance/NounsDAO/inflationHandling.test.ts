...
async function reset(): Promise<void> {
  ...
  const { address: timelockAddress } = await new thewordDaoExecutorFactory(deployer).deploy(
    govDelegatorAddress,
    timelockDelay,
  );
...
describe('thewordDAO#inflationHandling', () => {
  before(async () => {
  ...
  it('returns quorum votes and proposal threshold based on TheWord total supply', async () => {
    // Total Supply = 40
  ...
  it('rejects if proposing below threshold', async () => {
    ...
    ).revertedWith('thewordDAO::propose: proposer votes below proposal threshold');
  });
